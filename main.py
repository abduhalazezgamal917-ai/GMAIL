...
def callback_query(call):
    ...
    if call.data == "check_sub":
        ...
        try:
            ...
            print(f"[generate_email] error: {e}")
            bot.answer_callback_query(
                call.id,
                "❌ تعذر إنشاء البريد حاليًا، حاول مرة أخرى بعد قليل.",
                show_alert=True,
            )
            return
        ...
        if len(req) == 0:
            ...
            for msg in req:
                ...
                try:
                    ...
                    read_msg = read_resp.json()
                except (requests.RequestException, ValueError) as e:
                    print(f"[check_inbox] readMessage error: {e}")
                    bot.send_message(user_id, f"⚠️ تعذر جلب محتوى الرسالة رقم {msg_id}.")
                    continue

                sender = read_msg.get('from', 'غير معروف')
                subject = read_msg.get('subject', 'بدون عنوان')
                body = read_msg.get('textBody') or 'لا يوجد نص'
                if len(body) > 3000:
                    body = body[:3000] + "\n... (تم اقتصاص النص)"
